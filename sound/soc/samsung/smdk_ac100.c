...
//	printk("%s:bfs=%d\n",__func__,bfs);
	...
	ret = snd_soc_dai_set_clkdiv(codec_dai,0, 44100);
	if (ret < 0)
		return ret;
	...
	ret = snd_soc_dai_set_pll(codec_dai,0, AC100_MCLK1,	rclk, 22579200);
	if (ret < 0)
		return ret;

	ret = snd_soc_dai_set_sysclk(codec_dai, AC100_MCLK1,
	...
	writel(0x1, S5P_CLKSRC_AUDSS); // 44.1K
...
static struct snd_soc_dai_link smdk_dai[] = {
	{ /* Primary DAI i/f */
		...
		.codec_dai_name = "ac100_audio",	  
		.platform_name = "samsung-audio",	  
		.codec_name = "ac100.4-001a",		  
		.ops = &smdk_ops,
	},
		{ /* Primary DAI i/f */
		.name = "AC100 PAIF RX",	  
...
//
 //MODULE_DESCRIPTION("ALSA SoC SMDK WM8978");
 //MODULE_LICENSE("GPL");
