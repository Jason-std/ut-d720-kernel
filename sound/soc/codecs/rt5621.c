...
static struct rt5621_reg init_data[] = {
	...
	{RT5621_OUTPUT_MIXER_CTRL	, 0x1700},    //default output mixer control	
	{RT5621_ADC_REC_MIXER		, 0x1f1f},    //set record source is Mic1 by default
	{RT5621_ADC_REC_GAIN		, 0xfb94},	  //adc gain 18db	
	{RT5621_MIC_CTRL		, 0x0500},    //set Mic1,Mic2 boost 20db	
	{RT5621_MIC_ROUTING_CTRL, 0xf0f0},	//set mIc routing diff mode	
	{RT5621_SPK_OUT_VOL		, 0x8080},    //default speaker volume to 0db 
	{RT5621_HP_OUT_VOL		, 0x0808},    //default HP volume to -12db	
...
static const char *rt5621_voicecall_path[] = { "OFF", "RCV", "SPK", "HP","HP_NO_MIC", "BT","MainMic","HpMic" };

static const struct soc_enum rt5621_path_control_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(rt5621_voicecall_path),rt5621_voicecall_path),
 };
static void rt5621_call_off(struct snd_soc_codec *codec)
{
	rt5621_dbg("%s",__FUNCTION__);                
	return;                                       
}
static void rt5621_call_rcv(struct snd_soc_codec *codec)
{
	rt5621_dbg("%s",__FUNCTION__);      
	return;                                       
}
static void rt5621_call_spk(struct snd_soc_codec *codec)
{
 	int i=0;
	rt5621_dbg("%s",__FUNCTION__); 	
	  for(i=0;i<ARRAYSIZE(rt5621_spk_rcv);i++)
				 snd_soc_write(codec,rt5621_spk_rcv[i].reg,rt5621_spk_rcv[i].value);
	 return;									   

}
static void rt5621_call_hp(struct snd_soc_codec *codec)
{
	rt5621_dbg("%s",__FUNCTION__);      
	return;                                       

}
static void rt5621_call_hp_no_mic(struct snd_soc_codec *codec)
{
	rt5621_dbg("%s",__FUNCTION__);                
	return;                                       

}
static void rt5621_call_bt(struct snd_soc_codec *codec)
{
	rt5621_dbg("%s",__FUNCTION__);                
	return;                                       

}
 static void rt5621_record_mic(struct snd_soc_codec *codec)
 {
 	int i=0;
	rt5621_dbg("%s",__FUNCTION__); 	
	  for(i=0;i<ARRAYSIZE(rt5621_main_mic_reg);i++)
				 snd_soc_write(codec,rt5621_main_mic_reg[i].reg,rt5621_main_mic_reg[i].value);
	 return;									   
 
 }
      static void rt5621_record_hpmic(struct snd_soc_codec *codec)
 {
 	int i=0;
	rt5621_dbg("%s",__FUNCTION__); 	
	  for(i=0;i<ARRAYSIZE(rt5621_hp_mic_reg);i++)
				 snd_soc_write(codec,rt5621_hp_mic_reg[i].reg,rt5621_hp_mic_reg[i].value);
	 return;									   
 
 }                                                     

 static select_route uj_rt5621_voice_call_path[]={            
//	"OFF", 				"RCV",			"SPK", 						"HP",		 "HP_NO_MIC", 			"BT"  
 	rt5621_call_off,rt5621_call_rcv,	rt5621_call_spk,		rt5621_call_hp,rt5621_call_hp_no_mic,	rt5621_call_bt,rt5621_record_mic,rt5621_record_hpmic
 };                                                                                                    
static int rt5621_get_voice_call(struct snd_kcontrol *kcontrol,
...
static int rt5621_set_voice_call(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
		struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);  
		int path_num=ucontrol->value.integer.value[0];
		printk("%s\n",__FUNCTION__);
  	uj_rt5621_voice_call_path[path_num](codec);
	return 0;
}


static const struct snd_kcontrol_new rt5621_snd_controls[] = {
	SOC_DOUBLE_TLV("Speaker_Playback_Volume", RT5621_SPK_OUT_VOL,
		RT5621_L_VOL_SFT, RT5621_R_VOL_SFT, 31, 1, out_vol_tlv),
	SOC_DOUBLE("Speaker_Playback_Switch", RT5621_SPK_OUT_VOL,
		RT5621_L_MUTE_SFT, RT5621_R_MUTE_SFT, 1, 1),
	SOC_ENUM_EXT("Speaker_Amp_Type", rt5621_amp_type_enum,
		snd_soc_get_enum_double, rt5621_amp_sel_put),
	SOC_DOUBLE_TLV("Headphone_Playback_Volume", RT5621_HP_OUT_VOL,
		RT5621_L_VOL_SFT, RT5621_R_VOL_SFT, 31, 1, out_vol_tlv),
	SOC_DOUBLE("Headphone_Playback_Switch", RT5621_HP_OUT_VOL,
		RT5621_L_MUTE_SFT, RT5621_R_MUTE_SFT, 1, 1),
	SOC_DOUBLE_TLV("AXO_Playback_Volume", RT5621_MONO_AUX_OUT_VOL,
		RT5621_L_VOL_SFT, RT5621_R_VOL_SFT, 31, 1, out_vol_tlv),
	SOC_DOUBLE("AXO_Playback_Switch", RT5621_MONO_AUX_OUT_VOL,
		RT5621_L_MUTE_SFT, RT5621_R_MUTE_SFT, 1, 1),
	SOC_DOUBLE_TLV("PCM_Playback_Volume", RT5621_STEREO_DAC_VOL,
		RT5621_L_VOL_SFT, RT5621_R_VOL_SFT, 31, 1, dac_vol_tlv),
	SOC_DOUBLE_TLV("Line_In_Volume", RT5621_LINE_IN_VOL,
		RT5621_L_VOL_SFT, RT5621_R_VOL_SFT, 31, 1, in_vol_tlv),
	SOC_SINGLE_TLV("Mic_1_Volume", RT5621_MIC_VOL,
		RT5621_L_VOL_SFT, 31, 1, in_vol_tlv),
	SOC_SINGLE_TLV("Mic_1_Boost", RT5621_MIC_CTRL,
		RT5621_MIC1_BOOST_CTRL_SFT, 3, 0, mic_bst_tlv),
	SOC_SINGLE_TLV("Mic_2_Volume", RT5621_MIC_VOL,
		RT5621_R_VOL_SFT, 31, 1, in_vol_tlv),
	SOC_SINGLE_TLV("MIC_2_Boost", RT5621_MIC_CTRL,
		RT5621_MIC2_BOOST_CTRL_SFT, 3, 0, mic_bst_tlv),
	SOC_DOUBLE_TLV("Capture_Volume", RT5621_ADC_REC_GAIN,
		RT5621_ADCL_VOL_SFT, RT5621_R_VOL_SFT, 31, 0, adc_vol_tlv),

	
	//urbetter
	 SOC_DOUBLE("ADC_To_Hp_Mute", RT5621_ADC_REC_GAIN, 15, 14, 1, 0),
	 SOC_DOUBLE("ADC_To_Molo_Mute", RT5621_ADC_REC_GAIN, 13, 12, 1, 0),
	 SOC_DOUBLE("ADC_Dezero_Enable", RT5621_ADC_REC_GAIN, 6, 5, 1, 0),
	 SOC_DOUBLE("ADC_MIC_CTRL_BOOST", RT5621_MIC_CTRL, 10, 8, 3, 0),//00 bypass 01 +20db +30db 
	
	SOC_SINGLE("Mic1_CaptureL_Switch", RT5621_ADC_REC_MIXER,
		RT5621_M_MIC1_TO_ADC_L_MIXER_SFT, 1, 1),
	SOC_SINGLE("Mic2_CaptureL_Switch", RT5621_ADC_REC_MIXER,
		RT5621_M_MIC2_TO_ADC_L_MIXER_SFT, 1, 1),
	SOC_SINGLE("Mic1_CaptureR_Switch", RT5621_ADC_REC_MIXER,
		RT5621_M_MIC1_TO_ADC_R_MIXER_SFT, 1, 1),
	SOC_SINGLE("Mic2_CaptureR_Switch", RT5621_ADC_REC_MIXER,
		RT5621_M_MIC2_TO_ADC_R_MIXER_SFT, 1, 1),

//0x3e
	SOC_SINGLE("POWER_MAIN_BIAS",RT5621_PWR_MANAG_ADD3,15,1,0),
	SOC_DOUBLE("POWER_AUX_OUT_VOL",RT5621_PWR_MANAG_ADD3,14,13,1,0),
	SOC_SINGLE("POWER_SPK",RT5621_PWR_MANAG_ADD3,12,1,0),
	SOC_DOUBLE("POWER_HP_VOL",RT5621_PWR_MANAG_ADD3,10,9,1,0),
	SOC_DOUBLE("POWER_LINE_VOL",RT5621_PWR_MANAG_ADD3,7,6,1,0),
	SOC_DOUBLE("POWER_AUXIN_VOL",RT5621_PWR_MANAG_ADD3,5,4,1,0),	
	SOC_DOUBLE("POWER_MIC_VOL",RT5621_PWR_MANAG_ADD3,3,2,1,0),
    SOC_SINGLE("POWER_MIC_11",RT5621_PWR_MANAG_ADD3,11,1,0),
	
//0x10	
	SOC_SINGLE("MIC1_TO_HP_MUTE",RT5621_MIC_ROUTING_CTRL,15,1,0),
	SOC_SINGLE("MIC1_TO_SPK_MUTE",RT5621_MIC_ROUTING_CTRL,14,1,0),
	SOC_SINGLE("MIC1_TO_MONO_MUTE",RT5621_MIC_ROUTING_CTRL,13,1,0),
	SOC_SINGLE("MIC1_DIFF_CTRL",RT5621_MIC_ROUTING_CTRL,12,1,0),
	
	SOC_SINGLE("MIC2_TO_HP_MUTE",RT5621_MIC_ROUTING_CTRL,7,1,0),
	SOC_SINGLE("MIC2_TO_SPK_MUTE",RT5621_MIC_ROUTING_CTRL,6,1,0),
	SOC_SINGLE("MIC2_TO_MONO_MUTE",RT5621_MIC_ROUTING_CTRL,5,1,0),
	SOC_SINGLE("MIC2_DIFF_CTRL",RT5621_MIC_ROUTING_CTRL,4,1,0),
//0x1c
	SOC_SINGLE("CLSAB_AMP_SEL",RT5621_OUTPUT_MIXER_CTRL,12,1,0),
	SOC_SINGLE("SPK_VOL_IN_SEL",RT5621_OUTPUT_MIXER_CTRL,10,3,0),
	SOC_DOUBLE("HP_IN_SEL",RT5621_OUTPUT_MIXER_CTRL,9,8,1,0),
	SOC_SINGLE("MONO_IN_SEL",RT5621_OUTPUT_MIXER_CTRL,6,3,0),
//0x06
	SOC_DOUBLE("AUXO_MUTE",RT5621_MONO_AUX_OUT_VOL,15,7,1,0),
	SOC_DOUBLE("AUXO_DEZERO",RT5621_MONO_AUX_OUT_VOL,14,6,1,0),
	SOC_DOUBLE("AUXO_SOFT_MUTE_EN",RT5621_MONO_AUX_OUT_VOL,13,5,1,0),
	SOC_DOUBLE("AUXO_VOL",RT5621_MONO_AUX_OUT_VOL,8,0,32,0),

//0x08
	SOC_SINGLE("AUXI2HP_MUTE",RT5621_AUXIN_VOL,15,1,0),
	SOC_SINGLE("AUXI2SPK_MUTE",RT5621_AUXIN_VOL,14,1,0),
	SOC_SINGLE("AUXI2MONO_MUTE",RT5621_AUXIN_VOL,13,1,0),
	SOC_DOUBLE_TLV("AUX_In_Volume", RT5621_AUXIN_VOL,
			RT5621_L_VOL_SFT, RT5621_R_VOL_SFT, 31, 1, in_vol_tlv),
//0x3c
	SOC_SINGLE("POWER_MIX_MONO",RT5621_PWR_MANAG_ADD2,2,1,0),

	
	SOC_ENUM_EXT("Voice_Call_Path",rt5621_path_control_enum[0],rt5621_get_voice_call,rt5621_set_voice_call),
};
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	u16 dac_vol = snd_soc_read(codec, RT5621_STEREO_DAC_VOL);
	u16 mixer_vol = snd_soc_read(codec, RT5621_OUTPUT_MIXER_CTRL);
	u16 spk_vol = snd_soc_read(codec, RT5621_SPK_OUT_VOL);
	u16 hp_vol = snd_soc_read(codec, RT5621_HP_OUT_VOL);
		
			printk("rt5621_pcm_hw_params RT5621_AUDIO_INTERFACE 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x\n", 
					iface, dac_vol, mixer_vol, spk_vol, hp_vol);
			iface &= 0x7fff;
			dev_dbg(codec->dev, "enter %s\n", __func__);	
			if (coeff < 0) {
			...
			snd_soc_write(codec, RT5621_STEREO_DAC_VOL, dac_vol&0x1fff); //
			snd_soc_write(codec, RT5621_OUTPUT_MIXER_CTRL, 0x1700); //milo modify 0x4b00
		//		 snd_soc_write(codec, RT5621_SPK_OUT_VOL, 0x0000); //
		//	snd_soc_write(codec, RT5621_HP_OUT_VOL,  0x0000); //
			snd_soc_write(codec, RT5621_ADC_REC_MIXER,	0x3f3f);///milo add  
		/// snd_soc_write(codec, RT5621_JACK_DET_CTRL,	0x4000);
...
static int __devinit rt5621_i2c_probe(struct i2c_client *i2c,
...
static int __devexit rt5621_i2c_remove(struct i2c_client *i2c)
{
