...
#if defined( CONFIG_EXYNOS4X12_1500MHZ_SUPPORT	)|| defined( CONFIG_EXYNOS4X12_1600MHZ_SUPPORT)		
	{ 1325000, 1312500, 1300000, 1287500, 1300000, 1287500,	1275000, 1250000, 1250000, 1237500, 1225000, 1212500 },
#else	
...
//				pr_info("CPUFREQ L%d : %d uV\n", i, exynos4x12_volt_table[i]);
			...
			for (i = 0 ; i < CPUFREQ_LEVEL_END ; i++) {
				if (samsung_rev() >= EXYNOS4412_REV_2_0)
					exynos4x12_volt_table[i] =
						asv_voltage_step_12_5_rev2[i][exynos_result_of_asv] + CPU_MARGIN;
				else
					exynos4x12_volt_table[i] =
...
//add by jerry 
	if(!strncmp(g_selected_utmodel, "s702", strlen("s702"))
		...
		|| !strncmp(g_selected_utmodel, "s501", strlen("s501")))
		{
#if 0		
			memcpy(clkdiv_cpu0_4412, clkdiv_cpu0_4412_702, sizeof(clkdiv_cpu0_4412_702));
			memcpy(clkdiv_cpu1_4412, clkdiv_cpu1_4412_702, sizeof(clkdiv_cpu1_4412_702));
			memcpy(exynos4x12_apll_pms_table, exynos4x12_apll_pms_table_702, sizeof(exynos4x12_apll_pms_table_702));
//			printk("\n\nclkdiv_cpu0_4412[2][4] = %d\n\n\n", clkdiv_cpu0_4412[2][4]);			
#endif
		}
//add end
	...
	pr_info("Bootup CPU Frequency = [%d] %ldMHz\n", info->pm_lock_idx,
		rate / 1000);
