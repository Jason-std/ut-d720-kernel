...
static struct early_suspend     tmu_early_suspend;
#endif
...
/* added by Albert_lee 2014-04-25 begin */
		curr_temp_code = info->te1 + 5;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static struct s5p_tmu_info *g_tmu_info = NULL;
static void s5p_tmu_early_suspend(struct early_suspend *h) 
{
        struct s5p_tmu_info *info = g_tmu_info;
...
static void s5p_tmu_late_resume(struct early_suspend *h)
{
        struct s5p_tmu_info *info = g_tmu_info;
        struct s5p_platform_tmu *data = info->dev->platform_data;
...
#ifdef CONFIG_HAS_EARLYSUSPEND 
	/* disabled TMU irq in earlysuspend */ 
	/* because temp-sensor power is off on SC1 */
	g_tmu_info = info;
	tmu_early_suspend.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING + 10;
	tmu_early_suspend.suspend = s5p_tmu_early_suspend;
	tmu_early_suspend.resume = s5p_tmu_late_resume;
	register_early_suspend(&tmu_early_suspend);
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND 
	unregister_early_suspend(&tmu_early_suspend);
#endif
	cancel_delayed_work(&info->polling);
	destroy_workqueue(tmu_monitor_wq);
