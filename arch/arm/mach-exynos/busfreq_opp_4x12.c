...
#define MIF_MARGIN  (12500)
#define INT_MARGIN  (12500)
static void exynos4x12_set_bus_volt(void)
{
	...
	for (i = 0 ; i < LV_END ; i++) {
		...
		//jeff, for margin test
		exynos4_busfreq_table[i].volt += MIF_MARGIN;
		exynos4_int_volt[asv_group_index][i] += INT_MARGIN;
	}
...
//add by albert for test start 
	if (IS_ERR(opp))  
		opp = data->max_opp;
//add by albert for test end
