...
extern int s3c_pm_is_key_wakeup(void);
...
#define GPIO_HUB2_RESET EXYNOS4_GPL2(2)
*/
...
#if 1//jerry
/*
	printk("set HUB_RESET & HUB_CONNECT\n");

	// HUB_RESET
	gpio_request(GPIO_USB_HUB_RST, "GPIO_HUB_RESET");
	gpio_direction_output(GPIO_USB_HUB_RST, 1);
	
	s3c_gpio_setpull(GPIO_USB_HUB_RST, S3C_GPIO_PULL_NONE);
	gpio_free(GPIO_USB_HUB_RST);

	// HUB_CONNECT
	gpio_request(GPIO_USB_HUB_CONNECT, "GPIO_HUB_CONNECT");
	gpio_direction_output(GPIO_USB_HUB_CONNECT, 1);
	s3c_gpio_setpull(GPIO_USB_HUB_CONNECT, S3C_GPIO_PULL_NONE);
	gpio_free(GPIO_USB_HUB_CONNECT);


	gpio_request(GPIO_USB_HUB_INT, "HUB_INT");
