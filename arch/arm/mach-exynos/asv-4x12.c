...
int exynos4x12_asv_init(struct samsung_asv *asv_info)
{
	...
	tmp = __raw_readl(CHIP_ID_REG);	//TODO

	/* Store PKG_ID */
	asv_info->pkg_id = tmp;

	printk("\n\nCHIP_ID_REG=0x%08X, system_rev=%08x\n", tmp, system_rev);
...
/* act pmic OEM  by Albert_lee 2014-05-16 begin */
		if (is_act_pmu && !is_act_pmu_class2) {
			
			if(1==exynos_cal_asv) { // kk is not  confirm, TODO
				exynos_cal_asv = 3;
			}

			if(2==exynos_cal_asv) {  // KK is confirm
				exynos_cal_asv = 6;
			}

			if(exynos_cal_asv>=7 ) { // kk is not  confirm, TODO
				exynos_cal_asv = 4;
			}

			if(exynos_cal_asv==5 ) { // kk is confirm
				exynos_cal_asv = 6;
			}

			if(4==exynos_cal_asv ) { // kk is confirm
				exynos_cal_asv = 3;
			}
