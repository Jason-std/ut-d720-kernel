...
//#define BT_SLEEP_DBG
#ifdef BT_SLEEP_DBG
#define BT_DBG(fmt, arg...) printk(KERN_WARNING "%s: " fmt "\n", __func__, ##arg)
#else
...
#define BT2AP_WAKE GPIO_BT_HOST_WAKE
#define AP2BT_WAKE GPIO_BT_WAKE
...
/* added by Albert_lee 2014-04-01 begin */
	wake_lock_init(&bsi->wake_lock, WAKE_LOCK_SUSPEND, "bdwg");
 /*Albert_lee 2014-04-01 end*/
	wake_lock(&bsi->wake_lock);
...
//	bsi->host_wake = res->start;
	bsi->host_wake = BT2AP_WAKE;

	ret = gpio_request(bsi->host_wake, "bt_host_wake");
	if (ret) {
		BT_ERR("couldn't request host_wake gpio\n");
		goto free_bsi;
	}
    else {
		gpio_direction_input(bsi->host_wake);
		s3c_gpio_setpull(bsi->host_wake, S3C_GPIO_PULL_UP);
//		s3c_gpio_setpull(bsi->host_wake, S3C_GPIO_PULL_DOWN);
    }
...
/* modified by Albert_lee 2014-03-31 begin */
...
//	}
	...
	bsi->host_wake_irq = IRQ_BGF_INF; //gpio_to_irq(bsi->host_wake);
 /*Albert_lee 2014-03-31 end*/
...
static void __exit bluesleep_exit(void)
{
	...
	remove_proc_entry("sleep", bluetooth_dir);
	remove_proc_entry("bluetooth", 0);
	platform_device_unregister(&brcm_bluesleep_device);
...
MODULE_AUTHOR("<lisheng@urbetter.com>");
