		...
		int retry_counter)
{
	...
	printk(KERN_DEBUG"###### = udev->speed=%x\n", udev->speed);
	...
	if (udev->speed != USB_SPEED_SUPER)
		dev_info(&udev->dev,
				...
				devnum, udev->bus->controller->driver->name);  /// error .................
...
#define GET_DESCRIPTOR_BUFSIZE	64
			...
			for (j = 0; j < 3; ++j) {
				...
				default:
					if (r == 0) {
						printk(KERN_DEBUG"### %s(), err ret !!MixPacketsize[%d]\n", __func__, buf->bMaxPacketSize0);
						r = -EPROTO;
					}
					break;
				}
...
fail:
	...
	printk(KERN_DEBUG"### %s(), retval=0x%x. devnum=%d.   End !!\n", __func__, retval, devnum);
