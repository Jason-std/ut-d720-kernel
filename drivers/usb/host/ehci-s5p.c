...
#undef UT_EHCI_PATCH
#define UT_EHCI_PATCH  // important !!!!
	...
	struct usb_hcd *hcd)
{
	...
	if (pdata && pdata->noti_host_states) // now nc
...
static int s5p_wait_for_cp_resume_gsm (struct usb_hcd *hcd)
{
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
	u32 __iomem	*portsc ;
	u32 val32, retry_cnt = 0;
...
#if defined(CONFIG_LINK_DEVICE_USB)	//albert
...
#if defined(CONFIG_LINK_DEVICE_USB)	//albert
		printk("\n\n%s() ,  go \n", __func__);
		s5p_wait_for_cp_resume(pdev, hcd);  //albert
#endif
#ifdef CONFIG_USB_EXYNOS_SWITCH
	} else {
//		if (samsung_board_rev_is_0_0()) {
...
//#ifdef CONFIG_SMM6260_MODEM
	.wait_for_device	= s5p_wait_for_cp_resume_gsm,
//#endif
				...
				const char *buf, size_t count)
{
	...
	if (err < 2 || port < 0 || port > 3 || power_on < 0 || power_on > 1) {
		pr_err("port power fail: port_power 1 2(port %d enable 1)\n", port);
		return count;
	}
