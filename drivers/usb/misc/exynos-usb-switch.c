...
unsigned char host_device_state=0;
...
{
//    printk("liao test exynos_host_port_power_off\r\n");
				...
				enum usb_cable_status mode)
{
	...
	int ret = 0;
	printk("liao test exynos_change_usb_mode cur_mode=%d mode=%d\r\n",cur_mode,mode);
	if (cur_mode) {
...
static int exynos_usb_status_init(struct exynos_usb_switch *usb_switch)
{
	if (atomic_read(&usb_switch->connect))
		printk("liao usb ogt status Already setting\n");
		...
		if (usb_switch->gpio_host_vbus)
			...
			queue_delayed_work(usb_switch->workqueue, &usb_switch->monitor,1);
	} else
		atomic_set(&usb_switch->connect, 0);
...
static void exnos_usb_switch_worker(struct work_struct *work)
{
	...
	struct exynos_usb_switch *usb_switch =
		container_of(work, struct exynos_usb_switch, monitor.work);

	printk("liao worker gpio_host_detect+++++++++id=%d\r\n",gpio_get_value(usb_switch->gpio_host_detect));
	printk("liao worker gpio_device_detect+++++++++id=%d\r\n",gpio_get_value(usb_switch->gpio_device_detect));
...
//			printk("liao worker gpio_device_detect id=%d\r\n",gpio_get_value(usb_switch->gpio_device_detect));
		 ...
		 if((atomic_read(&usb_switch->connect) == 3) &&(!gpio_get_value(usb_switch->gpio_host_detect))) {
			if (usb_switch->gpio_host_vbus)
				 set_host_vbus(usb_switch, 0);
			msleep(100);
		 ...
		 } else if ((!gpio_get_value(usb_switch->gpio_device_detect))) {
			atomic_set(&usb_switch->connect,0);
			mdelay(100);
	...
	{
//			set_host_vbus(usb_switch, 0);//add by liao, otg power off when system auto wackup
//			printk("~~~~~~~~~~~~~~~~~~~~~\n");
		queue_delayed_work(usb_switch->workqueue, &usb_switch->monitor,80);
	}
...
//	mdelay(2000);
//	printk("**********%s************\n",__func__);
	printk("liao host gpio_host_detect id=%d\r\n",gpio_get_value(usb_switch->gpio_host_detect));
//	printk("liao host gpio_device_detect id=%d\r\n",gpio_get_value(usb_switch->gpio_device_detect));
...
//	printk("liao device gpio_host_detect id=%d\r\n",gpio_get_value(usb_switch->gpio_host_detect));
	printk("liao device gpio_device_detect id=%d\r\n",gpio_get_value(usb_switch->gpio_device_detect));

	mutex_lock(&usb_switch->mutex);
	if (is_host_detect(usb_switch)) {
		printk("liao usb otg Not expected situation\n");
	} else if (is_device_detect(usb_switch)) {
		if (usb_switch->gpio_host_vbus)
			exynos_change_usb_mode(usb_switch, USB_DEVICE_ATTACHED);
		else
...
static int exynos_usbswitch_suspend(struct device *dev)
{
	...
	printk(KERN_INFO "%s\n", __func__);
	disable_irq(usb_switch->device_detect_irq);
	disable_irq(usb_switch->host_detect_irq);
//	printk("**********%s************\n",__func__);

	cancel_delayed_work(&usb_switch->monitor);

	if (usb_switch->gpio_host_vbus)
		set_host_vbus(usb_switch, 0);//add by liao, otg power off when system auto wackup

//	printk("liao test cur_mode cur_mode==%d\r\n",cur_mode);
	atomic_set(&usb_switch->connect, atomic_read(&usb_switch->connect));
	return 0;
}
...
static int exynos_usbswitch_resume(struct device *dev)
{
	...
	printk(KERN_INFO "%s\n", __func__);
	printk("%s(); usb_switch=%d\r\n", __func__, atomic_read(&usb_switch->connect));
    queue_delayed_work(usb_switch->workqueue, &usb_switch->monitor,80);
	// exynos_usb_status_init(usb_switch);
	...
	enable_irq(usb_switch->device_detect_irq);
	enable_irq(usb_switch->host_detect_irq);
...
//	usb_switch->gpio_host_vbus = 0;
	...
	if (is_axp_pmu)
		usb_switch->gpio_host_vbus = GPIO_EXAXP22(4);
