...
int mpu_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	/* add by yangmiansi @2013-09-30 */
	/*printk("------mpu3050 i2c read something------\n");
	while (1)
	{
		int ires = i2c_smbus_write_byte_data(client, 0x04, 27);
		unsigned char res = i2c_smbus_read_byte_data(client, 0x04); 
		printk("------res = %x------\n", res);
	}*/
...
static int __init mpu_init(void)
{
	...
	if(!CHECK_UTMODEL("d1011")) {
		pr_info("%s: Probe name %s, not d1011 TODO !!!! \n", __func__, MPU_NAME);
		return -1;
	}
