...
static int bma250_read(void *mlsl_handle,
		       ...
		       unsigned char *data)
{
	//printk("------bma250_read() beginning------\n"); // add by yangmiansi @2013-09-30
	int result = INV_SUCCESS;
	...
	if (1) { /* KLP - workaroud for small data ready window */
		...
		//printk("---------%s---data=%d\n",__func__,(int)data); // add by yangmiansi @2013-09-30
		return result;
	} else
...
//add by eking for g-sensor orientation 20170719

extern char g_selected_utmodel[];

struct ext_slave_platform_data inv_mpu_bma250_data_s8001= {
...
struct ext_slave_platform_data inv_mpu_bma250_data_s7801= {
	.bus         = EXT_SLAVE_BUS_SECONDARY,
			   ...
			   const struct i2c_device_id *devid)
{
	...
	//printk("------bma250_mod_probe() beginning------\n"); // add by yangmiansi @2013-09-30
...
//add by eking for g-sensor orientation  20130719
#if 0
	...
	{
	       pdata = &inv_mpu_bma250_data_s8001;
      	}
	else if(strncmp(g_selected_utmodel, "s7801", strlen("s7801")) == 0)
	{
	       pdata = &inv_mpu_bma250_data_s7801;
      	} 
	else
		{
		pdata = client->dev.platform_data; 
...
//add end
	...
	//printk("------bma250_mod_probe() ending------\n"); // add by yangmiansi @2013-09-30
	return result;
...
{
	//printk("------bma250_mod_remove() beginning------\n"); //add by yangmiansi @2013-09-30
	...
	//printk("------bma250_mod_remove() ending------\n"); // add by yangmiansi @2013-09-30
...
static int __init bma250_mod_init(void)
{
	...
	res = i2c_add_driver(&bma250_mod_driver);
