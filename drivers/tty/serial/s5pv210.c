...
#ifdef CONFIG_PM
static int s5p_serial_suspend(struct platform_device *pdev, pm_message_t state)
{
#ifdef CONFIG_UART0_WORK_ALONE
	if (0 == pdev->id)
		return s3c24xx_uart0_suspend(pdev, state);
	else
#endif
		return s3c24xx_serial_suspend(pdev, state);
}

static int s5p_serial_resume(struct platform_device *pdev)
{
#ifdef CONFIG_UART0_WORK_ALONE
	if (0 == pdev->id)
		return s3c24xx_uart0_resume(pdev);
	else
#endif
		return s3c24xx_serial_resume(pdev);
}
...
#ifdef CONFIG_PM
	.suspend = s5p_serial_suspend,
	.resume = s5p_serial_resume,
#endif
	.driver		= {
		.name	= "s5pv210-uart",
		.owner	= THIS_MODULE,
