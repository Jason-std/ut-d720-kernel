...
static int cpufreq_hyper_governor_dbs(struct cpufreq_policy *policy,
				unsigned int event);
...
struct cpufreq_governor cpufreq_gov_hyper = {
	.name                   = "hyperfreq",
	.governor               = cpufreq_hyper_governor_dbs,
	.owner                  = THIS_MODULE,
};
...
struct workqueue_struct *hyper_dvfs_workqueue;
...
static void apply_hotplug_lock(void)
{
	...
	queue_work_on(dbs_info->cpu, hyper_dvfs_workqueue, work);
}

int cpufreq_hyper_cpu_lock(int num_core)
{
	int prev_lock;
...
int cpufreq_hyper_cpu_unlock(int num_core)
{
...
struct cpu_usage_history *hyper_hotplug_history;
				  ...
				  const char *buf, size_t count)
{
	...
	if (prev_lock)
		cpufreq_hyper_cpu_unlock(prev_lock);
	...
	ret = cpufreq_hyper_cpu_lock(input);
	if (ret) {
...
{
	int num_hist = hyper_hotplug_history->num_hist;
	struct cpu_usage *usage;
	int freq, rq_avg;
	int i;
	...
	for (i = num_hist - 1; i >= num_hist - up_rate; --i) {
		usage = &hyper_hotplug_history->usage[i];
	...
	if (min_freq >= up_freq && min_rq_avg > up_rq) {
		...
		hyper_hotplug_history->num_hist = 0;
		return 1;
	}
	return 0;
}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	hyper_hotplug_history->usage[num_hist].freq = policy->cur;
	hyper_hotplug_history->usage[num_hist].rq_avg = get_nr_run_avg();
	++hyper_hotplug_history->num_hist;
	...
	for_each_cpu(j, policy->cpus) {
		...
		hyper_hotplug_history->usage[num_hist].load[j] = load;
	...
	if (check_up()) {
		queue_work_on(this_dbs_info->cpu, hyper_dvfs_workqueue,
			      &this_dbs_info->up_work);
	} else if (check_down()) {
		queue_work_on(this_dbs_info->cpu, hyper_dvfs_workqueue,
			      &this_dbs_info->down_work);
	}
	if (hyper_hotplug_history->num_hist  == max_hotplug_rate)
		hyper_hotplug_history->num_hist = 0;
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, hyper_dvfs_workqueue, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, hyper_dvfs_workqueue,
...
unsigned int hyper_prev_freq_step;
unsigned int hyper_prev_sampling_rate;
static void cpufreq_hyper_early_suspend(struct early_suspend *h)
{
	dbs_tuners_ins.early_suspend =
		atomic_read(&g_hotplug_lock);
	hyper_prev_freq_step = dbs_tuners_ins.freq_step;
	hyper_prev_sampling_rate = dbs_tuners_ins.sampling_rate;
...
static void cpufreq_hyper_late_resume(struct early_suspend *h)
{
	...
	dbs_tuners_ins.freq_step = hyper_prev_freq_step;
	dbs_tuners_ins.sampling_rate = hyper_prev_sampling_rate;
	apply_hotplug_lock();
	start_rq_work();
}
#endif
...
#if defined( CONFIG_EXYNOS4X12_1600MHZ_SUPPORT)
	policy->max=1600000;	// add by albert 
#elif defined( CONFIG_EXYNOS4X12_1500MHZ_SUPPORT)
	policy->max=1500000;	// add by albert 
#endif
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	hyper_hotplug_history = kzalloc(sizeof(struct cpu_usage_history), GFP_KERNEL);
	if (!hyper_hotplug_history) {
		pr_err("%s cannot create hotplug history array\n", __func__);
		ret = -ENOMEM;
		goto err_hist;
	}

	hyper_dvfs_workqueue = create_workqueue("hyperfreq");
	if (!hyper_dvfs_workqueue) {
		pr_err("%s cannot create workqueue\n", __func__);
		ret = -ENOMEM;
		goto err_queue;
	}

	ret = cpufreq_register_governor(&cpufreq_gov_hyper);
	if (ret)
		goto err_reg;
...
err_reg:
	destroy_workqueue(hyper_dvfs_workqueue);
err_queue:
	kfree(hyper_hotplug_history);
err_hist:
	kfree(rq_data);
	return ret;
}
