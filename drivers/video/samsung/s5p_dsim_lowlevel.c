...
void lenove_s5p_dsim_enable_hs_clock(unsigned int dsim_base, unsigned char enable)
{
	unsigned int reg = (readl(dsim_base + S5P_DSIM_CLKCTRL)) &
...
//new add by xufei
unsigned int s5p_dsim_rd_rx_data(unsigned int dsim_base)
{
	return readl(dsim_base + S5P_DSIM_RXFIFO);
}
//new add by xufei
//from
void s5p_dsim_force_bta(unsigned int dsim_base)
{	
	writel(readl(dsim_base + S5P_DSIM_ESCMODE) | ( 0x1 << 16 ), dsim_base + S5P_DSIM_ESCMODE);
}

unsigned int s5p_dsim_get_intr_state(unsigned int dsim_base)
{
	return  readl(dsim_base + S5P_DSIM_INTSRC);
}

 void s5p_dsim_force_d_phy_stop_state(unsigned int dsim_base, u8 ucEn)
{
	u32 uTemp = readl(dsim_base + S5P_DSIM_ESCMODE) & ~( 0x1 << 20 );
	uTemp |= ((ucEn & 0x1 )<< 20);
	writel( uTemp, dsim_base + S5P_DSIM_ESCMODE);
}
//end