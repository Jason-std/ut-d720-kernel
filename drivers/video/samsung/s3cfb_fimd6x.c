...
#define VSTATUS_IS_BACK(reg)		(reg == VIDCON1_VSTATUS_BACKPORCH)
...
{
	u32 cfg = (readl(ctrl->regs + S3C_VIDCON1) & (3 << 13));
	if (cfg != (2 << 13) && cfg != (1 << 13))
		return 1;
	else
		return 0;
}
...
//get FIMD data transmitting status:active ? hbp ? hfp?
static unsigned int get_vstatus(struct s3cfb_global *ctrl)
{
	u32 reg;

	reg = readl(ctrl->regs + S3C_VIDCON1);
	reg &= VIDCON1_VSTATUS_MASK;
	return reg;
}

int s3cfb_set_clock_down(struct s3cfb_global *ctrl,unsigned long mode)
{
	struct s3c_platform_fb *pdata = to_fb_plat(ctrl->dev);
	u32 cfg, maxclk, src_clk, vclk, div;
    u8 status =1;
    spinlock_t      slock;// new add by xufei
    u32 mask = reg_mask(6,8);//shift = 6 bit;size = 8 bit
    ...
    if( mode ) {  
        vclk = (vclk*2)/3;// 60hz --> 40hz
        //vclk =  vclk/2;//60hz --> 30hz
    }
    ...
    printk("start to set FIMD frame again\n");
    do {
            //printk("start to adjust FIMD frame \n");
            //printk("div is %d\n",div);
            //spin_lock_irqsave(&slock, flags);
   
            if (div == g_div) {//如果预设值就是正在使用的值
                //spin_unlock_irqrestore(&slock, flags);
                printk("=======================same div value!!=======================\n");
                return -EINVAL;
            } 
            #if 0
            ...
            writel(cfg, ctrl->regs + S3C_VIDCON0);
            //spin_unlock_irqrestore(&slock, flags);              
            printk("FREQ_DEBUG===============###set FIMD VLCK again sucessfully!!###===================\n");
            return 0;
            #endif
          
            if (VSTATUS_IS_ACTIVE(get_vstatus(ctrl))) {              
                //printk("status is active!\n");
                if (VSTATUS_IS_FRONT(get_vstatus(ctrl))) {
                    //hfp status:write the div value to register
                    g_div = div;
                    //printk("status is hfp!\n");
                    cfg &= ~S3C_VIDCON0_CLKVAL_F(0xff);
	                cfg |= S3C_VIDCON0_CLKVAL_F(div - 1);
	                writel(cfg, ctrl->regs + S3C_VIDCON0);
                    
                    //spin_unlock_irqrestore(&slock, flags);              
                    /*printk("=======================###set FIMD VLCK again sucessfully!!###===================\n");
            ...
            //printk("end adjust FIMD frame\n");
    
       } while (count);
    
       printk("===========================================>err: %s skip, div=%d\n", __func__, div);
...
static int set_div(struct device *dev, u32 div)
{
	...
	//struct clksrc_clk *clksrc = g_info->clksrc;
	...
	do {
		spin_lock_irqsave(&g_info->slock, flags);
		reg = __raw_readl(clksrc->reg_div.reg);

		if ((reg & mask) == (div & mask)) {//如果预设值就是正在使用的值
			spin_unlock_irqrestore(&g_info->slock, flags);
			return -EINVAL;
		}

		reg &= ~(0xff);
		reg |= div;//设置div到分频寄存器中的[分频值段]
...
int s3cfb_set_clock(struct s3cfb_global *ctrl)
{
    ...
    static unsigned char once_done =1;
	/* spec is under 100MHz */
	maxclk = 100 * 1000000;
...
//	cfg |= S3C_VIDTCON2_HOZVAL(ctrl->lcd->width - 1); //lt,mr1
