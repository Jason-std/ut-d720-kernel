...
{
	int ret=0;
	ret = i2c_smbus_write_byte_data(bdi->client, reg, data);
	if(ret!=0)
		printk("%s,failed\n",__func__);
	return ret;
}
...
static ssize_t bq24190_sysfs_show_ut(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t bq24190_sysfs_store_ut(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct bq24190_sysfs_field_info bq24190_sysfs_field_tbl[] = {
	...
	{									\
	.attr	= __ATTR(ut_debug, S_IWUSR | S_IRUGO, bq24190_sysfs_show_ut, bq24190_sysfs_store_ut),	\
	.reg	= 0,					\
	.mask	= 0,				\
	.shift	= 0,			\
	},
		...
		struct device_attribute *attr, char *buf)
{
	...
	for(i=0;i<0x0b;i++){
		bq24190_read(bdi,i,&v);
		scnprintf(buf+i*15, PAGE_SIZE, "reg[0x%02x]=0x%02x\n",i,v);
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	{
		bq24190_write(bdi,reg, value);
	}
	else if(sscanf(buf, "%x", &reg) == 1)
	{
		bq24190_read(bdi,reg,&value);
	}
...
static int bq24190_hw_init(struct bq24190_dev_info *bdi)
{
	...
	ret = bq24190_read_mask(bdi, BQ24190_REG_VPRS,
			...
			&v);
	if (ret < 0){
		printk("%s,read mask failed\n",__func__);
		goto out;
	}

	if (v != bdi->model) {
		ret = -ENODEV;
		printk("%s,model error,v=%d\n",__func__,v);
		goto out;
	}

	ret = bq24190_register_reset(bdi);
	if (ret < 0){
		printk("%s,register reset failed\n",__func__);
		goto out;
	}

	ret = bq24190_set_mode_host(bdi);
	printk("%s,set mode host ret= %d\n",__func__,ret);
out:
	pm_runtime_put_sync(bdi->dev);
	return ret;
}
...
static u8 reg_init[8]={0x4f,0x1c,0x9c,0x51,0xaf,0x88,0x01,0x43};
...
*/
	...
	for(i=0;i<8;i++)
		bq24190_write(bdi, i, reg_init[i]);
...
static struct i2c_board_info bq24190_i2c_board_info[] __initdata = {
	{
...
static int __init bq24190_init(void)
{

	int ret;
	ret = i2c_add_driver(&bq24190_driver);
	printk("%s:ret=%d\n",__func__,ret);
	return ret;

}

static void __exit bq24190_exit(void)
{
	i2c_del_driver(&bq24190_driver);
}

module_init(bq24190_init);
module_exit(bq24190_exit);
