...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if ((finger == 0x00) && (CHIP_TYPE_GT9F == ts->chip_type))     // request arrived
    {
        ...
        case GTP_RQST_CONFIG:
			...
			//msleep(500);
			//gtp_i2c_read(ts->client,buf,3);
			//printk(">> gtp_send_cfg ,read i2c buf[0]= 0x%x,buf[1]= 0x%x,buf[2]= 0x%x",buf[0],buf[1],buf[2]);
            if (ret < 0)
            {
            ...
            else
            {
                ...
                GTP_INFO("Request for config responded!  rqst_buf[2]=0x%x",rqst_buf[2]);//*
            }
            break;
