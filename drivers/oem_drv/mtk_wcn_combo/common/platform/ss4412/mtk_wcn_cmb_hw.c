...
mtk_wcn_cmb_hw_pwr_off (VOID)
{
    ...
    iRet += wmt_plat_sdio_ctrl(WMT_SDIO_SLOT_SDIO1, FUNC_OFF);
...
//iRet += wmt_plat_gpio_ctrl(PIN_GPS_LNA, PIN_STA_INIT);
...
#if 0 /* RTC source is enabled by cpu rtc */
    ...
    if (_pwr_first_time) {
        ...
        WMT_INFO_FUNC("rtc clock exported\n");
        _pwr_first_time = 0;
    }
#endif
    ...
    /* irq enable/disabled is controlled by host controller driver, ex. sdhci. */
    iRet += wmt_plat_gpio_ctrl(PIN_WIFI_EINT, PIN_STA_MUX);


    iRet += wmt_plat_sdio_ctrl(WMT_SDIO_SLOT_SDIO1, FUNC_ON);
	
    WMT_INFO_FUNC("WIFI_INT mux to EINT mode\n");
