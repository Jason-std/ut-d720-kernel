...
#define BATTERY_UP_MAX_CHANGE   420             // the max time allow battery change quantity(当充电器在的时候，防止不能满充出现)
#define BATTERY_DOWN_CHANGE   60                // the max time allow battery change quantity（强制满充时，往下修的值）
#define BATTERY_DOWN_MIN_CHANGE_RUN 30          // the min time allow battery change quantity when run（防止抖动？）
