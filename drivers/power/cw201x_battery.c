...
#define REG_VERSION			 0x0 //d
#define REG_VCELL			   0x2	//VOL
#define REG_SOC				 0x4  //剩余容量
#define REG_RRT_ALERT		   0x6 //时钟和报警位，报警位必须由HOST发起，写1清0？
#define REG_CONFIG			  0x8 //update config+ATH
#define REG_MODE				0xA  //QSTART PRO SLEEP
#define REG_BATINFO			 0x10  //64 byte BATINFO

#define MODE_SLEEP_MASK		 (0x3<<6)
#define MODE_SLEEP			  (0x3<<6) //MCU power down
#define MODE_NORMAL			 (0x0<<6) //00 to wake up
#define MODE_QUICK_START		(0x3<<4) //firmware restart
#define MODE_RESTART			(0xf<<0) //pro
...
// the max time allow battery change quantity(当充电器在的时候，防止不能满充出现)
#define BATTERY_DOWN_CHANGE   60
// the max time allow battery change quantity（强制满充时，往下修的值）
#define BATTERY_DOWN_MIN_CHANGE_RUN 30
// the min time allow battery change quantity when run（防止抖动？）
#define BATTERY_DOWN_MIN_CHANGE_SLEEP 1800	  //
#define BATTERY_DOWN_MAX_CHANGE_RUN_AC_ONLINE 1800	  //

#define NO_STANDARD_AC_BIG_CHARGE_MODE 1  //无标准的大电流充电时，写1
...
//extern int dwc_otg_check_dpdm(void); //check USB充电器
//extern int get_gadget_connect_flag(void); //check
//extern int dwc_vbus_status( void ); //外部AC插入
...
static int cw_read(struct i2c_client *client, u8 reg, char *buf)
{
	int ret;
...
static int cw_write(struct i2c_client *client, u8 reg, const char *buf)
{
	int ret;
...
static int cw_update_config_info(struct cw_battery *cw_bat)
{
	...
	"cw_bat->plat_data->cw_bat_config_info[%d] = 0x%x\n", i,  \
		...
		ret = cw_write(cw_bat->client, REG_BATINFO + i, &cw_bat->plat_data
	->cw_bat_config_info[i]);
...
static int cw_quickstart(struct cw_battery *cw_bat)
{
	...
	ret = cw_write(cw_bat->client, REG_MODE, &reg_val);
	//(MODE_QUICK_START | MODE_NORMAL));  // 0x30
	if (ret < 0)
	{
...
static int cw_get_capacity(struct cw_battery *cw_bat)
{
	...
	(cw_capacity > (cw_bat->capacity - 9))) || ((cw_bat->charger_mode == 0) &&
	...
	if ((cw_bat->charger_mode > 0) && (cw_capacity >= 95) && (cw_capacity <=
	cw_bat->capacity))
	{
		// avoid no charge full

		capacity_or_aconline_time = (cw_bat->sleep_time_capacity_change > cw_bat
	->sleep_time_charge_start) ? cw_bat->sleep_time_capacity_change: cw_bat
	->sleep_time_charge_start;
		capacity_or_aconline_time += (cw_bat->run_time_capacity_change > cw_bat
	->run_time_charge_start) ? cw_bat->run_time_capacity_change: cw_bat
	->run_time_charge_start;
		allow_change = (new_sleep_time + new_run_time - capacity_or_aconline_time) /
	...
	else if ((cw_bat->charger_mode == 0) && (cw_capacity <= cw_bat->capacity) &&
	...
	{
		charge_time = new_sleep_time + new_run_time - cw_bat->sleep_time_charge_start
	- cw_bat->run_time_charge_start;
		if ((charge_time > BATTERY_DOWN_MAX_CHANGE_RUN_AC_ONLINE) && (if_quickstart
	== 0))
		{
	...
	{
		if (gpio_get_value(cw_bat->plat_data->chg_ok_pin) != cw_bat->plat_data
	->chg_ok_level)
		{
			if (cw_capacity == 100)
			{
	...
	if ((cw_bat->charger_mode == 0) && (cw_capacity <= 20) && (cw_bat->voltage <=
	SYSTEM_SHUTDOWN_VOLTAGE))
	{
		if (if_quickstart == 10)
		{
			...
			dev_info(&cw_bat->client->dev,"the cw201x voltage is less than "
					"SYSTEM_SHUTDOWN_VOLTAGE !!!!!!!, funciton: %s, line: %d\n",
					__func__, __LINE__);
	}
...
static void rk_bat_update_capacity(struct cw_battery *cw_bat)
{
	...
	if ((cw_capacity >= 0) && (cw_capacity <= 100) && (cw_bat->capacity !=
	cw_capacity))
	{
...
static int get_usb_charge_state(struct cw_battery *cw_bat)
{
	...
	if (cw_bat->usb_online == 1)
	{
		...
		charge_time = time_from_boot - cw_bat->sleep_time_charge_start -
			cw_bat->run_time_charge_start;
		if (charge_time > 3)
		{
	...
	dev_dbg(&cw_bat->client->dev,
		"%s usb_status=[%d],cw_bat->charger_mode=[%d],cw_bat->gadget_status=[%d], "
		"cw_bat->charger_init_mode = [%d]\n", __func__, usb_status,
		cw_bat->charger_mode, gadget_status, cw_bat->charger_init_mode);
...
static int rk_usb_update_online(struct cw_battery *cw_bat)
{
	...
	if (usb_status == 2)
	{
		...
		{
			if (gpio_get_value(cw_bat->plat_data->chg_mode_sel_pin) != cw_bat->plat_data
	->chg_mode_sel_level)
				gpio_direction_output(cw_bat->plat_data->chg_mode_sel_pin, (cw_bat
	->plat_data->chg_mode_sel_level == GPIO_HIGH) ? GPIO_HIGH: GPIO_LOW);
		}
	...
	->chg_mode_sel_level)
				gpio_direction_output(cw_bat->plat_data->chg_mode_sel_pin, (cw_bat
	->plat_data->chg_mode_sel_level == GPIO_HIGH) ? GPIO_LOW: GPIO_HIGH);
		}
		if (cw_bat->usb_online != 1)
		{
			cw_bat->usb_online = 1;
...
static void cw_bat_work(struct work_struct *work)
{
	...
	dev_dbg(&cw_bat->client->dev,"cw_bat->bat_change = %d, cw_bat->time_to_empty"
		" = %d, cw_bat->capacity = %d, cw_bat->voltage = %d, cw_bat->dc_online = %d,"
		" cw_bat->usb_online = %d\n", cw_bat->bat_change, cw_bat->time_to_empty,
		cw_bat->capacity, cw_bat->voltage,	cw_bat->dc_online, cw_bat->usb_online);
}

static int rk_usb_get_property(struct power_supply *psy, enum
...
static int rk_ac_get_property(struct power_supply *psy, enum
	power_supply_property psp, union power_supply_propval *val)
{
	int ret = 0;
	struct cw_battery *cw_bat;
...
static int rk_battery_get_property(struct power_supply *psy, enum
	power_supply_property psp, union power_supply_propval *val)
{
	int ret = 0;
	struct cw_battery *cw_bat;
...
static int cw_bat_gpio_init(struct cw_battery *cw_bat)
{
	...
	if (cw_bat->plat_data->dc_det_pin != INVALID_GPIO)
	{
		...
		s3c_gpio_setpull(cw_bat->plat_data->dc_det_pin, S3C_GPIO_PULL_UP);
	...
	if (cw_bat->plat_data->bat_low_pin != INVALID_GPIO)
	{
		...
		s3c_gpio_setpull(cw_bat->plat_data->bat_low_pin, S3C_GPIO_PULL_UP);
	...
	if (cw_bat->plat_data->chg_ok_pin != INVALID_GPIO)
	{
		...
		s3c_gpio_setpull(cw_bat->plat_data->chg_ok_pin, S3C_GPIO_PULL_UP);
	...
	if ((cw_bat->plat_data->is_usb_charge == 1) &&
			(cw_bat->plat_data->chg_mode_sel_pin != INVALID_GPIO))
	{
		...
		ret = gpio_direction_output(cw_bat->plat_data->chg_mode_sel_pin,
			(cw_bat->plat_data->chg_mode_sel_level == GPIO_HIGH) ?
			GPIO_LOW: GPIO_HIGH);
		if (ret)
		{
	...
	request_chg_ok_pin_fail: if (cw_bat->plat_data->bat_low_pin != INVALID_GPIO)
		gpio_free(cw_bat->plat_data->bat_low_pin);

	request_bat_low_pin_fail: if (cw_bat->plat_data->dc_det_pin != INVALID_GPIO)
		gpio_free(cw_bat->plat_data->dc_det_pin);

	request_dc_det_pin_fail: return ret;
...
static void dc_detect_do_wakeup(struct work_struct *work)
{
	...
	type = gpio_get_value(cw_bat->plat_data->dc_det_pin) ? IRQ_TYPE_EDGE_FALLING :
...
{
	0x15, 0x54, 0x7C, 0x72, 0x66, 0x65, 0x59, 0x59, 0x57, 0x58, 0x54, 0x50,
	0x4E,0x49, 0x46, 0x40, 0x3A, 0x38, 0x2B, 0x1D, 0x21, 0x1B, 0x22, 0x15,
	0x24,0x1F, 0x0D, 0x71, 0x57, 0x87, 0x8D, 0x8B, 0x8E, 0x8D, 0x91, 0x90,
	0x30,0x30, 0x8E, 0x02, 0x03, 0x2B, 0x52, 0x87, 0x8F, 0x91, 0x94, 0x52,
	0x82,0x8C, 0x92, 0x96, 0x78, 0x75, 0x75, 0xCB, 0x2F, 0x7D, 0x72, 0xA5,
	0xB5,0xC1, 0xAB, 0x29,
};

static u8 cw201x_config_info_d1011[SIZE_BATINFO] =
{
	0x15 , 0x7C , 0x56 , 0x57 , 0x55 , 0x54 , 0x58 , 0x4D ,
	0x4B , 0x4B , 0x46 , 0x45 , 0x43 , 0x3D , 0x34 , 0x2F ,
	0x27 , 0x20 , 0x1F , 0x1D , 0x23 , 0x34 , 0x3C , 0x4E ,
	0x32 , 0x44 , 0x0C , 0xCD , 0x35 , 0x55 , 0x53 , 0x63 ,
	0x68 , 0x64 , 0x63 , 0x67 , 0x43 , 0x1C , 0x5A , 0x22 ,
	0x0A , 0x29 , 0x52 , 0x87 , 0x8F , 0x91 , 0x94 , 0x52 ,
	0x82 , 0x8C , 0x92 , 0x96 , 0x7B , 0x86 , 0xB2 , 0xCB ,
	0x2F , 0x7D , 0x72 , 0xA5 , 0xB5 , 0xC1 , 0xAB , 0x31 ,

};

static int cw_bat_probe(struct i2c_client *client, const struct i2c_device_id
	*id)
{
	struct cw_battery *cw_bat;
	int ret;
	...
	else if(CHECK_UTMODEL("d1011")){
		cw_bat->plat_data->cw_bat_config_info = cw201x_config_info_d1011;
	}
	else if (CHECK_UTMODEL("d521")){
		cw_bat->plat_data->dc_det_level=0;
	}
	...
	if (cw_bat->plat_data->dc_det_pin != INVALID_GPIO)
	{
		...
		irq = gpio_to_irq(cw_bat->plat_data->dc_det_pin);
#if 0
		irq_flags = gpio_get_value(cw_bat->plat_data->dc_det_pin) ?
	...
	IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_DISABLED | IRQF_ONESHOT,
	"dc_detect", cw_bat);
		if (ret < 0)
		{
			pr_err("%s: request_irq(%d) failed\n", __func__, irq);
	...
	if (cw_bat->plat_data->bat_low_pin != INVALID_GPIO)
	{
		...
		ret = request_irq(irq, bat_low_detect_irq_handler, IRQF_TRIGGER_RISING,
	...
	rk_usb_register_fail: power_supply_unregister(&cw_bat->rk_bat);
	rk_ac_register_fail: power_supply_unregister(&cw_bat->rk_ac);
	rk_bat_register_fail: dev_info(&cw_bat->client->dev,
...
static const struct i2c_device_id cw_id[] =
{
	{
		"cw201x", 0
	} ,  //cw2013 & cw2015 compatible
};
MODULE_DEVICE_TABLE(i2c, cw_id);

static const struct dev_pm_ops cw_bat_pm_ops =
{
	.suspend = cw_bat_suspend, .resume = cw_bat_resume,
};
#endif
...
static int __init cw_bat_init(void)
{
	...
	if (strstr(g_device_gauge, "cw2"))
	{
		return i2c_add_driver(&cw_bat_driver);
	}

	printk("%s(); - not used cw2 .\n", __func__);
	return  - 1;
}
