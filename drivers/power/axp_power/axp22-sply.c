...
/* added by Albert_lee 2013-07-16 begin */
static int bat_cap = 0;
static int bat_rdc = 0;
...
static  int __init select_battery_cap(char *str)
{
	...
	printk(KERN_DEBUG"%s: str=%s, bat_cap=%d\n", __func__, str, bat_cap);

	return 0;
}
__setup("batcap=",select_battery_cap);

static  int __init select_battery_rdc(char *str)
{
	if(strlen(str)>0) {
		sscanf(str, "%d", &bat_rdc);
	}

	if (bat_cap<=0)
		bat_cap = BATCAP;

	printk(KERN_DEBUG"%s: str=%s, bat_cap=%d\n", __func__, str, bat_rdc);

	return 0;
}
__setup("batrdc=",select_battery_rdc);
 /*Albert_lee 2013-07-16 end*/
...
#ifdef CONFIG_UT_ADC_BATTERY
	axp_charger_status = charger->ext_valid;
#endif
}
  ...
  if (charger->bat_det) {
    if (charger->ext_valid){
	  ...
	  if(strstr(g_selected_utmodel,"d720")){
	  	//	  printk("%s:power on green\n",__func__);
            gpio_direction_output(EXYNOS4212_GPM4(3),0);  //red
            gpio_direction_output(EXYNOS4212_GPM4(2),1);  //green
	  }
	}
    	else if(charger->charge_on){
    		val->intval = POWER_SUPPLY_STATUS_CHARGING;
		if(strstr(g_selected_utmodel,"d720")){
		   //printk("%s:power on red\n",__func__);
...
#ifdef CONFIG_UT_ADC_BATTERY
	axp_battery_status = val->intval;
#endif
}

#ifdef CONFIG_UT_ADC_BATTERY
int axp_charger_online(void) {	// 1:online,0:not noline;
	return (axp_charger_status);
}
EXPORT_SYMBOL(axp_charger_online);


int axp_is_bat_full(void)	// ericli add.1:full,0:not full
{
	return (axp_battery_status == POWER_SUPPLY_STATUS_FULL);
}

EXPORT_SYMBOL(axp_is_bat_full);
...
*/
  ...
  case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
  	if (bat_cap>0) {
		charger->battery_info->energy_full_design = bat_cap;
  	}
    val->intval = charger->battery_info->energy_full_design;
//	printk("POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:%d\n",val->intval);
...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	axp_read(charger->master, AXP22_CAP,&val);
	charger->rest_vol	= (int)	(val & 0x7F);
/* modified by Albert_lee 2013-09-29 begin */
	if (strncmp(g_selected_utmodel, "s503", strlen("s503"))==0) {
		low_pow_level = 45;
	}
//	printk(">>>>>>>>>> charger->rest_vol = %d \n", charger->rest_vol);

//	if(charger->rest_vol < low_pow_level/*35*/)	{
//		g_tmu_powersave |= USED_KFC_LOW_POWER;
//	} else
//		g_tmu_powersave &= ~USED_KFC_LOW_POWER;
 /*Albert_lee 2013-09-29 end*/
	if(axp_debug){
...
static void axp_usb(struct work_struct *work)
{
	...
	if(axp_usbcurflag){
		if(axp_debug)
		{
			printk(KERN_DEBUG"set usbcur_pc %d mA\n",USBCURLIMPC);
		}
		if(USBCURLIMPC){
			axp_clr_bits(charger->master, AXP22_CHARGE_VBUS, 0x01);
		...
		if(axp_debug)
		{
			printk(KERN_DEBUG"set usbcur %d mA\n",USBCURLIM);
		}
		if((USBCURLIM) && (USBCURLIMEN)){
			axp_clr_bits(charger->master, AXP22_CHARGE_VBUS, 0x01);
			var = USBCURLIM * 1000;
	...
	if(axp_usbvolflag){
		if(axp_debug)
		{
			printk(KERN_DEBUG"set usbvol_pc %d mV\n",USBVOLLIMPC);
		}
		if(USBVOLLIMPC){
		    axp_set_bits(charger->master, AXP22_CHARGE_VBUS, 0x40);
		...
		if(axp_debug)
		{
			printk(KERN_DEBUG"set usbvol %d mV\n",USBVOLLIM);
		}
		if((USBVOLLIM) && (USBVOLLIMEN)){
		    axp_set_bits(charger->master, AXP22_CHARGE_VBUS, 0x40);
		  	var = USBVOLLIM * 1000;
...
static int axp_battery_probe(struct platform_device *pdev)
{
  ...
  printk("%s:bat_rdc=%d,bat_cap=%d\n",__func__,bat_rdc,bat_cap);
  ...
  printk("axp charger not limit now pdata->chgcur=%d\n", pdata->chgcur);
...
*/
  ...
  ocv_cap[18] = pocv[9];
  ocv_cap[19] = 0xCA;
  ocv_cap[20] = pocv[0x0a];
  ocv_cap[21] = 0xCB;
  ocv_cap[22] = pocv[0x0b];
  ocv_cap[23] = 0xCC;
  ocv_cap[24] = pocv[0x0c];
  ocv_cap[25] = 0xCD;
  ocv_cap[26] = pocv[0x0d];
  ocv_cap[27] = 0xCE;
  ocv_cap[28] = pocv[0x0e];
  ocv_cap[29] = 0xCF;
  ocv_cap[30] = pocv[0x0f];
  ocv_cap[31] = 0xD0;
  ocv_cap[32] = pocv[0x10];
  ocv_cap[33] = 0xD1;
  ocv_cap[34] = pocv[0x11];
  ocv_cap[35] = 0xD2;
  ocv_cap[36] = pocv[0x12];
  ocv_cap[37] = 0xD3;
  ocv_cap[38] = pocv[0x13];
  ocv_cap[39] = 0xD4;
  ocv_cap[40] = pocv[0x14];
  ocv_cap[41] = 0xD5;
  ocv_cap[42] = pocv[0x15];
  ocv_cap[43] = 0xD6;
  ocv_cap[44] = pocv[0x16];
  ocv_cap[45] = 0xD7;
  ocv_cap[46] = pocv[0x17];
  ocv_cap[47] = 0xD8;
  ocv_cap[48] = pocv[0x18];
  ocv_cap[49] = 0xD9;
  ocv_cap[50] = pocv[0x19];
  ocv_cap[51] = 0xDA;
  ocv_cap[52] = pocv[0x1a];
  ocv_cap[53] = 0xDB;
  ocv_cap[54] = pocv[0x1b];
  ocv_cap[55] = 0xDC;
  ocv_cap[56] = pocv[0x1c];
  ocv_cap[57] = 0xDD;
  ocv_cap[58] = pocv[0x1d];
  ocv_cap[59] = 0xDE;
  ocv_cap[60] = pocv[0x1e];
  ocv_cap[61] = 0xDF;
  ocv_cap[62] = pocv[0x1f];
...
/* added by Albert_lee 2013-09-29 begin */
//	axp_set_bits(charger->master, AXP22_DCDC_MODESET,0x04);
 /*Albert_lee 2013-09-29 end*/
...
/* added by Albert_lee 2013-08-08 begin */
	axp_clr_bits(charger->master,AXP22_HOTOVER_CTL, 0x80);
 /*Albert_lee 2013-08-08 end*/

	if (TEMP_PROTECT)
		axp_set_bits(charger->master,AXP22_HOTOVER_CTL,0x04);
	else
		axp_clr_bits(charger->master,AXP22_HOTOVER_CTL,0x04);

/* RDC initial */
	axp_read(charger->master, AXP22_RDC0,&val2);
/* modified by Albert_lee 2013-07-16 begin */
#if 1
	printk("%s(), bat_rdc=%d.\n", __func__, bat_rdc);

	if((bat_rdc) && (!(val2 & 0x40)))		//如果配置电池内阻，则手动配置
	{
		rdc = (bat_rdc * 10000 + 5371) / 10742;
...
//probe 时初始化RDC，使其提前计算正确的OCV，然后在此处启动计量系统
	...
	if((bat_cap) /*&& (!(val2 & 0x80))*/)
	{
		Cur_CoulombCounter = bat_cap * 1000 / 1456;
...
static int axp22_suspend(struct platform_device *dev, pm_message_t state)
{
    ...
    axp_unregister_notifier(charger->master, &charger->nb, AXP22_NOTIFIER_ON);
    axp_register_notifier(charger->master, &charger->nb, AXP22_NOTIFIER_SUSPEND);
...
static int axp22_resume(struct platform_device *dev)
{
    ...
    axp_unregister_notifier(charger->master, &charger->nb, AXP22_NOTIFIER_SUSPEND);
...
/*====ericli add for close blacklight before shutdown 2013-09-24======== */
//	gpio_request_one(EXYNOS5410_GPB2(0), GPIOF_OUT_INIT_LOW, "GPB2");
////	gpio_set_value(EXYNOS5410_GPB2(0), 0);
//	gpio_direction_output(EXYNOS5410_GPB2(0), 0);
//
...
static int axp_battery_init(void)
{
	...
	if (is_axp_pmu && !is_axp_pmu_nocharger) {

	} else {
		printk("%s(); not match error !\n", __func__);
		return -1;
	}
