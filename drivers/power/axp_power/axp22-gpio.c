...
static int __devinit axp_gpio_probe(struct platform_device *pdev)
{
	...
	struct regulator *device_1V0_off_regulator = NULL;
	struct regulator *device_1V8_regulator = NULL;

	device_1V0_off_regulator = regulator_get(NULL, "vdd_10off");
	regulator_enable(device_1V0_off_regulator);
	regulator_put(device_1V0_off_regulator);

	device_1V8_regulator = regulator_get(NULL, "vdd_18off");
...
/* added by Albert_lee 2013-07-04 begin */

//	gpio_request(GPIO_AXP22(0), "GPIO_AXP22_0");
//	gpio_request(GPIO_AXP22(1), "GPIO_AXP22_1");
	gpio_request(GPIO_AXP22(2), "GPIO_AXP22_2");
	gpio_request(GPIO_AXP22(3), "GPIO_AXP22_3");
	gpio_request(GPIO_AXP22(4), "GPIO_AXP22_4");


	gpio_direction_output(GPIO_AXP22(2), 0);
	gpio_direction_output(GPIO_AXP22(4), 0);
	printk("%s(); gpio4=%x \n", __func__, ret);

//	gpio_free(GPIO_AXP22(0));
//	gpio_free(GPIO_AXP22(1));
	gpio_free(GPIO_AXP22(2));
	gpio_free(GPIO_AXP22(3));
	gpio_free(GPIO_AXP22(4));
 /*Albert_lee 2013-07-04 end*/


	ret =axp_read(&axp->dev, AXP22_PWREN_CTRL1,&val);
	printk("%s(); 8C=%x \n", __func__, val);
	ret =axp_read(&axp->dev, AXP22_PWREN_CTRL2,&val);
	printk("%s(); 8D=%x \n", __func__, val);

#ifdef CONFIG_AXP_DVT_VER
	ret =axp_write(&axp->dev, AXP22_PWREN_CTRL1, (u8)( ~((1<<2)|(1<<7))));
	ret =axp_write(&axp->dev, AXP22_PWREN_CTRL2, (u8)( ~((1<<2)|(1<<3))));
#else
	ret =axp_write(&axp->dev, AXP22_PWREN_CTRL1, (u8)( ~((1<<0)|(1<<3)|(1<<7))));
	ret =axp_write(&axp->dev, AXP22_PWREN_CTRL2, (u8)( ~((1<<1)|(1<<0)|(1<<2)|(1<<3))));
#endif
...
static void axp_gpio_shutdown(struct platform_device *pdev)
{
	printk("%s(); +\n", __func__);

	gpio_request(GPIO_AXP22(4), "USB_HOST_PW_EN");

	gpio_direction_output(GPIO_AXP22(4),  0);
	gpio_set_value(GPIO_AXP22(4), 0);
	gpio_free(GPIO_AXP22(4));
	printk("%s(); -\n", __func__);
