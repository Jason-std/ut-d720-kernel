...
#undef CONFIG_AXP_CHGCHANGE
#undef CONFIG_HAS_EARLYSUSPEND
#undef CONFIG_AXP_CHARGEINIT
...
//  printk("add axp_battery_event to notifier[%2x]\n", axp_battery_event);
  charger->nb.notifier_call = axp_battery_event;
     ret = axp_register_notifier(charger->master, &charger->nb, AXP22_NOTIFIER_ON_NOCHARGER);   	
...
err_ps_register:
    axp_unregister_notifier(charger->master, &charger->nb, AXP22_NOTIFIER_ON_NOCHARGER);
...
//    if (skip_charger==1) {
        axp_register_notifier(charger->master, &charger->nb, AXP22_NOTIFIER_ON_NOCHARGER);
//    } else {	
...
static int axp_battery_init(void)
{
	...
	if ( is_axp_pmu_nocharger ) { // 108 no charger
