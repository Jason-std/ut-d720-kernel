...
EXPORT_SYMBOL(sdhci_s3c_notify_change);
...
static int __devinit sdhci_s3c_probe(struct platform_device *pdev)
{
    ...
    if (/*2 == host->mmc->index*/(host->mmc->pm_flags & MMC_PM_IGNORE_SUSPEND_RESUME)) {
	
        /* to avoid redundant mmc_detect_change() called by mmc_pm_notify() */
        printk(KERN_INFO "%s: set MMC_PM_IGNORE_PM_NOTIFY for %s pm_flags=[%08x]\n",
            __func__, mmc_hostname(host->mmc), host->mmc->pm_flags);
...
static struct platform_driver sdhci_s3c_driver = {
	...
	.shutdown	= sdhci_s3c_shutdown,  //add by albert for poweroff sdio
	.driver		= {
		.owner	= THIS_MODULE,
		.name	= "s3c-sdhci",
	},
