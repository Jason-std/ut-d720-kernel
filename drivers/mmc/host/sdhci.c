...
static unsigned int suspended_flag2 = 0;
static unsigned int suspended_flag3 = 0;
...
// Cellon add start, Ted Shi, 2012/11/02, for sd resume crash
	...
	if(suspended_flag2 && !strcmp(mmc_hostname(host->mmc),"mmc2")){
		cmd->error = -EIO;
		tasklet_schedule(&host->finish_tasklet);
		return;
	}

	if(suspended_flag3 && !strcmp(mmc_hostname(host->mmc),"mmc3")){
		cmd->error = -EIO;
		tasklet_schedule(&host->finish_tasklet);
		return;
	}
...
// Cellon modify start, Ted Shi, 2012/11/09, for samsung mmc patch
	...
	mod_timer(&host->timer, jiffies + 3 * HZ);	// [yeez_xiaoyi] less wait time
...
static void sdhci_set_clock(struct sdhci_host *host, unsigned int clock)
{
...
#if 0// add by albert for force change sdio clk
	...
	if (host->version >= SDHCI_SPEC_300/* || 2 == host->mmc->index|| 3 == host->mmc->index*/) {
		/*
...
/*
		if(2 == host->mmc->index && timeout<1) {
			printk(KERN_ERR"  >> ");
//			msleep(1);
//			sdhci_suspend_host(host, tmp);
//			msleep(1);
...
/*
		if((!strncmp(dev_name(mmc_dev(host->mmc)),"s3c-sdhci.3",sizeof("s3c-sdhci.3")))&&(flag_wifi1))
		{
			present =  true;
		}
		else
...
//		if (host->vmmc) {
			regulator_disable(host->vmmc); // albert test
		...
		if (host->vmmc && !regulator_is_enabled(host->vmmc)) {
//		if (host->vmmc ) {
			regulator_enable(host->vmmc); // albert test
...
#if MMC2_SDIO_EXT_IRQ
    if (is_mtk_combo() && (2 == mmc->index) && (ios->power_mode == MMC_POWER_OFF)) {
...
//    mmc2_sdio_ext_irq = IRQ_WIFI_WUP /*gpio_to_irq(MMC2_SDIO_EINT_PIN)*/;
//    mmc2_sdio_ext_irq = gpio_to_irq(MMC2_SDIO_EINT_PIN);
    mmc2_sdio_ext_irq = IRQ_EINT(MMC2_SDIO_EINT_IRQ);
...
//    irq_set_irq_wake(mmc2_sdio_ext_irq, 1); //add by albert
...
/*Shawn added start for card detect*/
...
#if MMC2_SDIO_EXT_IRQ
    if (is_mtk_combo() && /*(2 == host->mmc->index)*/(host->mmc->pm_flags & MMC_PM_IGNORE_SUSPEND_RESUME)
...
#if MMC2_SDIO_EXT_IRQ
//    if (2 == host->mmc->index) {
//	printk("##%s() pm_flags=%x \n", __func__, host->mmc->pm_flags);
//    }

    if (is_mtk_combo() &&(2 == host->mmc->index)
        && (host->mmc->pm_flags & MMC_PM_KEEP_POWER)) {
    ...
    if (is_mtk_combo() &&(2 == host->mmc->index) && !(host->flags & SDHCI_DEVICE_DEAD)) {
        if (0 != mmc2_sdio_ext_irq) {
