 ...
 void dump_region(struct fimc_is_dev *dev)
{
 ...
 void fimc_is_err_print(struct fimc_is_dev *dev)
 {
 	printk(KERN_ERR "fimc_is_err_print  start!! \n");
	 unsigned int LP, CP;
	 mDebugVaddr=phys_to_virt(dev->mem.base + FIMC_IS_DEBUG_REGION_ADDR);
 ...
 mCurrWp = mIs_debug_ctrl.write_point;
	 ...
	 printk(KERN_ERR "fimc_is_err_print  end!! \n");
...
void fimc_is_hw_open_sensor(struct fimc_is_dev *dev, u32 id, u32 sensor_index)
{
	...
	printk("lzy~~sensor_index=%d",sensor_index);
	switch (sensor_index) {
/*
...
*/
	...
	case SENSOR_S5K3H7_CSI_A:
		...
		sensor_ext = (struct sensor_open_extended *)
		 ...
		 sensor_ext->actuator_type.name= 2;
              sensor_ext->actuator_type.type = SE_I2C;    
              sensor_ext->actuator_type.setting.i2c.channel = 0;
#ifdef CONFIG_MACH_CYIT_EVT
              sensor_ext->flash_type.name= 2; 
#endif
#ifdef CONFIG_MACH_SC1_DVT1 
	      sensor_ext->flash_type.name = 1;
	      sensor_ext->flash_type.type = SE_GPIO;
	      sensor_ext->flash_type.setting.gpio.first_gpio_port_no= 8;
	      sensor_ext->flash_type.setting.gpio.second_gpio_port_no=9;   
#endif

              sensor_ext->fRom_type.name = 100;
		...
		case SENSOR_S5K3H7_CSI_B:
		//for front 4h5
		printk("In Front4h5   SENSOR_S5K3H7_CSI_B");
		sensor_ext = (struct sensor_open_extended *)
						&dev->is_p_region->shared;
		sensor_ext->actuator_type.name= 100;//2;
              //sensor_ext->actuator_type.type = SE_I2C;    
              //sensor_ext->actuator_type.setting.i2c.channel = 0;
#ifdef CONFIG_MACH_CYIT_EVT
              sensor_ext->flash_type.name= 2; 
