...
#define FRONT_HI253_MODULE_BUG //jmq.the HI253 module has problems if the rear sensor uses same MCLK with it. 
...
struct fimc_is_dev {
	...
	struct regulator		*r_fvddio18_cam;//for front 4H5 
	struct regulator		*r_fvdd28_af_cam;
	struct regulator		*r_fvadd28_cam;
	struct regulator		*r_vdd18_mipi_tv;
	struct regulator		*r_vdd10_mipi_tv;
#ifdef CONFIG_MACH_SC1_DVT1 
#ifdef FRONT_HI253_MODULE_BUG
	struct regulator		*r_vdd18_2m;
	struct regulator		*r_vdd12_2m;
	struct regulator		*r_vdd28_2m;
#endif
#endif
};
...
extern void fimc_is_err_print(struct fimc_is_dev *dev);
extern   void dump_region(struct fimc_is_dev *dev);
