...
#define GPIO_CAM_RST_3H7 EXYNOS4_GPF1(5)
#define GPIO_CAM_RST_FRONT_4H5 EXYNOS4_GPF0(5)
//#define GPIO_CAM_RST_3H7 EXYNOS4_GPL0(1)
//#define GPIO_CAM_PWDN_VCM_3H7 EXYNOS4_GPX3(3)
#ifdef CONFIG_MACH_SC1_DVT1
...
#define GPIO_CAM_FRONT_4H5_MCLK    EXYNOS4212_GPM2(2)
...
#endif
#ifdef CONFIG_MACH_SC1_DVT1
#ifdef FRONT_HI253_MODULE_BUG
static int Front_power_regurator(struct fimc_is_dev *dev, int get)
{
        //TODO: to check which power pin exactly.
        int ret =0;
        if(get){
       	dev->r_vdd18_2m= regulator_get(NULL, "vdd18_2m");
       	if (IS_ERR(dev->r_vdd18_2m)) {
       		printk("%s: failed to get %s\n", __func__, "vdd18_2m");
       		ret = -1;
       	}
       	
       	dev->r_vdd12_2m= regulator_get(NULL, "vdd12_2m");
       	if (IS_ERR(dev->r_vdd12_2m)) {
       		printk("%s: failed to get %s\n", __func__, "vdd12_2m");
       		ret = -1;
       	}
       
       	dev->r_vdd28_2m= regulator_get(NULL, "vdd28_2m");
       	if (IS_ERR(dev->r_vdd28_2m)) {
       		printk("%s: failed to get %s\n", __func__, "vdd28_2m");
       		ret = -1;
       	}
	 }else{
		if (dev->r_vdd18_2m)
			regulator_put(dev->r_vdd18_2m);

		if (dev->r_vdd12_2m)
			regulator_put(dev->r_vdd12_2m);

		if (dev->r_vdd28_2m) 
			regulator_put(dev->r_vdd28_2m);
	 }
	 return ret;
}
static void Front_power(struct fimc_is_dev *is_dev,int poweron)
{
	printk("HI253 power %s in rear camera\n",poweron?"on":"off");
       if(poweron)
	{	
		regulator_enable(is_dev->r_vdd28_2m); 
		udelay(10);
		regulator_enable(is_dev->r_vdd12_2m);
		udelay(10);
		regulator_enable(is_dev->r_vdd18_2m); 
		udelay(10);

	}else{
		regulator_disable(is_dev->r_vdd28_2m); 
		regulator_disable(is_dev->r_vdd12_2m);
		regulator_disable(is_dev->r_vdd18_2m); 		
	}
}
#endif
#endif
...
static int fimc_is_probe(struct platform_device *pdev)
{
	...
	*/
...
#ifdef CONFIG_MACH_SC1_DVT1 
#ifdef FRONT_HI253_MODULE_BUG	
       if(Front_power_regurator(dev,1) !=0 )
	   	goto p_err_init_mem;
#endif
#endif

	printk("### regulator_get 1.2V 1.8V 2.8V for 3H7 sensor in %s###",__FUNCTION__);
	dev->r_vdd18_cam = regulator_get(NULL, "vdd12_5m");//SC1: SW6 ==>BDVDD_1.2V,FDVDD_1.2V
	if (IS_ERR(dev->r_vdd18_cam)) {
		pr_err("failed to get resource %s\n", "vdd12_5m");
		goto p_err_init_mem;
	}

	dev->r_vddio18_cam = regulator_get(NULL, "vdd12_2m");//SC1:LDO22==>BCAM_DVDDIO_1.8V
	...
	dev->r_vdd28_af_cam = regulator_get(NULL, "vdd28_af");//SC1:LDO21==>BAF_VCC_2.8V
	...
	dev->r_vadd28_cam = regulator_get(NULL, "vdd28_cam");//SC1:LDO20==>BAVDD_2.8V
...
//add for front 4h5 power supply
	dev->r_fvddio18_cam = regulator_get(NULL, "vdd18_2m");//SC1:LDO26==>FCAM_DVDDIO_1.8V
	if (IS_ERR(dev->r_fvddio18_cam)) {
		pr_err("failed to get resource %s\n", "vdd18_2m");
	...
	dev->r_fvdd28_af_cam = regulator_get(NULL, "faf_vcc_28");//SC1:LDO5==>FAF_VCC_2.8V
	if (IS_ERR(dev->r_fvdd28_af_cam)) {
		pr_err("failed to get resource %s\n", "faf_vcc_28");
		regulator_put(dev->r_fvddio18_cam);
		regulator_put(dev->r_vadd28_cam);
		regulator_put(dev->r_vdd28_af_cam);
		regulator_put(dev->r_vddio18_cam);
		regulator_put(dev->r_vdd18_cam);
	...
	dev->r_fvadd28_cam = regulator_get(NULL, "vdd28_2m");//SC1:LDO19==>FAVDD_2.8V
	if (IS_ERR(dev->r_fvadd28_cam)) {
		pr_err("failed to get resource %s\n", "vdd28_2m");
		regulator_put(dev->r_fvdd28_af_cam);
		regulator_put(dev->r_fvddio18_cam);
		regulator_put(dev->r_vadd28_cam);
		regulator_put(dev->r_vdd28_af_cam);
		regulator_put(dev->r_vddio18_cam);
...
//
	...
	if (IS_ERR(dev->r_vdd18_mipi_tv)) {
		pr_err("failed to get resource %s\n", __func__, "vdd18_mipi");
		regulator_put(dev->r_fvadd28_cam);
		regulator_put(dev->r_fvdd28_af_cam);
		regulator_put(dev->r_fvddio18_cam);
		regulator_put(dev->r_vdd28_af_cam);
		regulator_put(dev->r_vddio18_cam);
...
//for front 4H5
	if (dev->r_fvadd28_cam)
		regulator_put(dev->r_fvadd28_cam);
	
	if (dev->r_fvdd28_af_cam)
		regulator_put(dev->r_fvdd28_af_cam);
	
	if (dev->r_fvddio18_cam)
		regulator_put(dev->r_fvddio18_cam);
//
...
static int fimc_is_runtime_suspend(struct device *dev)
{
...
#endif
#if 0//def CONFIG_EXYNOS4_CPUFREQ
	...
	mutex_unlock(&is_dev->lock);
#if 0//def CONFIG_REGULATOR	
	printk("---power down sensor in %s,cam_id=%d",__FUNCTION__,cam_id);
if(cam_id==0){
	if (gpio_request(GPIO_CAM_RST_3H7, "GPF1_5") < 0)
		printk("failed gpio_request(GPF1_5) for camera control\n");
...
// for front 4h5
	if (gpio_request(GPIO_CAM_RST_FRONT_4H5, "GPF0_5") < 0)
		printk("failed gpio_request(GPF0_5) for camera control\n");
	gpio_direction_output(GPIO_CAM_RST_FRONT_4H5, 0);	
	gpio_free(GPIO_CAM_RST_FRONT_4H5);
	
	regulator_disable(is_dev->r_vdd18_cam);
	regulator_disable(is_dev->r_fvdd28_af_cam);
	regulator_disable(is_dev->r_fvddio18_cam);
	regulator_disable(is_dev->r_fvadd28_cam);
	regulator_disable(is_dev->r_vdd18_mipi_tv);
	regulator_disable(is_dev->r_vdd10_mipi_tv);
}
...
static int fimc_is_runtime_resume(struct device *dev)
{
	...
	//printk("---power up sensor in %s,cam_id=%d",__FUNCTION__,cam_id);
#if 0//def CONFIG_REGULATOR	
if(cam_id==0){//FOR REAR 4H5
	...
	s3c_gpio_setpull(GPIO_CAM_FLASH_3H7, S3C_GPIO_PULL_DOWN);
	gpio_free(GPIO_CAM_FLASH_3H7);	
	   ...
	   s3c_gpio_setpull(GPIO_CAM_TORCH_3H7, S3C_GPIO_PULL_DOWN);
	   gpio_free(GPIO_CAM_TORCH_3H7); 
	   mdelay(1);

	if (gpio_request(GPIO_CAM_RST_3H7, "GPF1_5") < 0)//3h7 reset
	...
	s3c_gpio_cfgpin(GPIO_CAM_FRONT_4H5_MCLK, S3C_GPIO_SFN(3));//MCLK

	 int ret = gpio_request(EXYNOS4212_GPM4(2), "GPM4_2");//IIC clock
	if (ret)
	...
	ret = gpio_request(EXYNOS4212_GPM4(3), "GPM4_3");//IIC data
	if (ret)
	...
	gpio_free(GPIO_CAM_FLASH_3H7);	
	   ...
	   if (gpio_request(GPIO_CAM_RST_FRONT_4H5, "GPF0_5") < 0)//3h7 reset
	printk("failed gpio_request(GPIO_CAM_RST_FRONT_4H5) for camera control\n");
	gpio_direction_output(GPIO_CAM_RST_FRONT_4H5, 0);
	s3c_gpio_setpull(GPIO_CAM_RST_FRONT_4H5, S3C_GPIO_PULL_NONE); 
	mdelay(1);
	
	regulator_enable(is_dev->r_vdd18_cam);//the regulator is the same with rear sensor
	regulator_enable(is_dev->r_fvdd28_af_cam);
	regulator_enable(is_dev->r_fvddio18_cam);
	regulator_enable(is_dev->r_fvadd28_cam);

	regulator_enable(is_dev->r_vdd18_mipi_tv);
	regulator_enable(is_dev->r_vdd10_mipi_tv);

	mdelay(1);
	gpio_direction_output(GPIO_CAM_RST_FRONT_4H5, 1);	
	mdelay(1);
	gpio_free(GPIO_CAM_RST_FRONT_4H5);
