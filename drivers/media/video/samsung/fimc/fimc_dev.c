...
extern int cam_id;
extern  int fimc_configure_subdev(struct fimc_control *ctrl);
int fimc_check_camera(struct fimc_global *fimc, struct fimc_control *ctrl, unsigned int i)
{
	struct s3c_platform_fimc *pdata = to_fimc_plat(ctrl->dev);
	int ret = 0;
	cam_id=i;
...
/**************** add by Leslie 2014-8-16 ****************/
	if((pdev->id == 0)&& !CHECK_UTMODEL("d721")){
		for(i=0,j=0;i<FIMC_MAXCAMS;i++){
			ret=fimc_check_camera(fimc_dev,ctrl,i);
			if(ret==0){
				fimc_dev->camera[j] = fimc_dev->camera[i];
				fimc_dev->camera_isvalid[j] = 1;
				fimc_dev->camera[j]->initialized = 0;
				printk("%d find camera %s\n",j,fimc_dev->camera[j]->info->type);
				j++;
			}
		}
		for(i=j;i<FIMC_MAXCAMS;i++){
			fimc_dev->camera[i] = NULL;
			fimc_dev->camera_isvalid[i] = 0;
		}
	}
/**************** add by Leslie 2014-8-16 end****************/
