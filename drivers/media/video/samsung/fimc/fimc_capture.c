...
static int fimc_capture_scaler_info(struct fimc_control *ctrl)
{
	...
	if(ctrl->cam != NULL)
	ctrl->cap->rotate = ctrl->cam->layout_rotate;//lzy. rotate according to sensor output	
...
//	} 
	...
	printk("%s zoomhor=%d,zoomver=%d\n",__func__,zoom_hor,zoom_ver);
	...
	if (ctrl->cam->window.height > ctrl->cam->height)
