...
extern void headset_irq_for_hdmi(void); //jodon for headset when plutout
void s5p_hdmi_ctrl_stop(void)
{
	struct s5p_hdmi_ctrl_private_data	*ctrl = &s5p_hdmi_ctrl_private;
	hdmi_plug_flag=0;
	headset_irq_for_hdmi();//Jodon add for hdmi sound when plug out
//	gpio_direction_output(EXYNOS4212_GPM1(4),1);
    write_power_item_value(POWER_SPK_EN,1); // enable spk
	tvout_dbg("running(%d)\n", ctrl->running);
	if (ctrl->running) {
		ctrl->running = false;
	...
	enum s5p_tvout_disp_mode disp, enum s5p_tvout_o_mode out)
{
	...
	write_power_item_value(POWER_SPK_EN,0); // disable spk
	ctrl->out = out;
