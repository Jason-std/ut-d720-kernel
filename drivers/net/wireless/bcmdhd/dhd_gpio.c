...
int bcm_wlan_get_oob_irq(void)
{
	...
	host_oob_irq = IRQ_WIFI_WUP; //gpio_to_irq(EXYNOS4_GPX0(7));
	s3c_gpio_setpull(GPIO_WIFI_WUP, S3C_GPIO_PULL_DOWN);
	gpio_direction_input(GPIO_WIFI_WUP);
...
void bcm_wlan_power_on(int flag)
{
	if (flag == 1) {
		...
		gpio_direction_output(GPIO_WIFI_REG_ON, 1);
		gpio_set_value(GPIO_WIFI_REG_ON, 1);
		mdelay(100);
		printk("======== Enable sdhci presence change! ========\n");
		mmc_force_presence_change(1);
	}
	else {
...
void bcm_wlan_power_off(int flag)
{
	if (flag == 1) {
		...
		mmc_force_presence_change(0);
		mdelay(100);
		printk("======== PULL WL_REG_ON LOW! ========\n");
		gpio_direction_output(GPIO_WIFI_REG_ON, 0);
		gpio_set_value(GPIO_WIFI_REG_ON, 0);
		mdelay(100);
	}
	else {
